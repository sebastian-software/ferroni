...
match and tokenizing lines than Oniguruma. The advantage holds across all three
production grammars, from roughly an order of magnitude to much larger wins on
TypeScript first-match latency. Compiling the full TypeScript grammar is also
faster in Ferroni, while Oniguruma still starts faster on the smaller Rust and
CSS grammars. In practice, Ferroni's strongest fit is a scanner that compiles a
grammar once and uses it repeatedly.
...
Compile-time results are mixed. Ferroni compiles the full TypeScript grammar
faster, but Oniguruma has an edge on Rust and CSS grammar startup and on several
