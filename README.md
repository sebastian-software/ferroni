...
With the `match-cache` Cargo feature, `RegexBuilder::match_cache` and
`Scanner::with_match_cache` can memoize failed states of eligible patterns.
The cache stays off unless explicitly configured and uses a shared 16 MiB
budget per scanner by default. Unsupported constructs and exhausted budgets
fall back to ordinary backtracking, so keep search limits enabled.
See the [`match_cache` module](https://docs.rs/ferroni/latest/ferroni/match_cache/)
for examples and the precise eligibility and lifetime rules. Independent
review and measured Ferriki adoption remain tracked in
[#163](https://github.com/sebastian-software/ferroni/issues/163).
...
30x faster on CSS
([exact numbers](https://sebastian-software.github.io/ferroni/perf/benchmark-results#scanner-on-whole-documents-line-by-line)).
...
([exact numbers](https://sebastian-software.github.io/ferroni/perf/benchmark-results#scanner-with-full-shiki-textmate-grammars)).
...
**2,333 `#[test]` functions** in total (some compat functions bundle multiple
...
| [015](https://sebastian-software.github.io/ferroni/adr/015-unicode-data-from-ucd) | Generate Unicode tables from the UCD |
