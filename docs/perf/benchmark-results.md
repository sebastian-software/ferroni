...
```bash
# User-facing C-comparison suite (scanner + text scanning)
cargo bench --features ffi

# Fast smoke comparison (5-10 core kernels, reduced warmup/sample)
FERRONI_BENCH_SMOKE=1 cargo bench --features ffi

# Full detail suite for engine compatibility and regressions
FERRONI_BENCH_DETAILED=1 cargo bench --features ffi

# Rust-only CI-tracked series
