...
[`regex` deliberately excludes lookarounds and backreferences](https://docs.rs/regex/1.13.1/regex/).
...
The [shared-syntax appendix](#shared-syntax-context-only-regex) retains every
...
point estimate than C in all seven cases, by **1.08–1.98x**. The scanner
speedups are not a useful estimate for ordinary regex work.
...
point estimate between Ferroni and C, not statistical significance. The validators repeat eight
...
Access-log capture and URL extraction were repeated separately with the same
configuration. Ferroni remained faster than C on these two inputs. The
appendix retains the third engine's repeat timings as shared-syntax context.
...
**Bold** marks the lower Ferroni/C point estimate, not statistical significance.
Factors are C time divided by Ferroni time; values above one favor Ferroni.
