...
Ferroni continues [Oniguruma](https://github.com/kkos/oniguruma) in Rust. The
C engine behind TextMate grammars, jq, and PHP's mbregex ended in April 2025;
Ferroni is a line-by-line port of it. It keeps the C engine's feature set and
its pattern semantics for ASCII and UTF-8 -- if a pattern works in Oniguruma,
it works in Ferroni -- and ships a multi-pattern Scanner API of the same shape
as vscode-oniguruma's for TextMate grammar tokenization. There is no C toolchain and no FFI
...
Ferroni's MSRV is Rust 1.94, enforced by a dedicated CI lane. The library has
no build script. The optional
...
// Unicode properties -- 902 property names are supported.
...
Lookbehind validation follows Oniguruma's parser, including its restrictions on
nested lookbehind. For example, Oniguruma rejects the positive form below with
an invalid-lookbehind syntax error, and Ferroni preserves that behavior:
...
The superficially similar outer negative form `(?<!(?<!a)b)c` is accepted.
Ferroni keeps this distinction for compatibility with the C engine.
...
`Scanner::new` and the default `ScannerConfig` enable capture groups, matching
vscode-oniguruma. When a pattern mixes named and unnamed groups, all groups
are returned and numbered backreferences work. If you relied on the former
Ferroni default, pass `ONIG_OPTION_NONE` explicitly; capture indices after a
named group may shift when unnamed groups are captured.

Watch for zero-width matches when you write that loop: if a pattern can match
the empty string, advance to the next character boundary instead of standing
still. Advancing by a single byte would split a multibyte character and panic
the next time you slice the line.
