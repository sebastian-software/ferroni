...
### Internal suite: `regression_bench`

Ferroni-only micro-benchmarks measured locally with Criterion. Compare saved baselines to check for performance regressions before merging changes. They are intentionally internal-facing: useful for optimizing parser, executor, scanner, RegSet, and public API paths, but not meant for README marketing tables.
...
- **Two suites separate concerns.** `battle_bench` is small, stable, and publishable; `regression_bench` is free to optimize for regression coverage and engineering feedback.
