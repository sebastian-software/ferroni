...
- **Validation and measurements**: [Unicode interval search refinement](/perf/unicode-range-lookup-refinement), including an exhaustive Unicode-range sweep, malformed table bounds, cache/plain/C checks, rejected alternatives, and a repeated 41-case before/after matrix. The earlier partition-search evaluations remain linked as historical evidence.
