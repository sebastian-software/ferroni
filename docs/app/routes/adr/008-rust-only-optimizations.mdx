...
- **Measurements**: [match cache evaluation](/perf/match-cache-evaluation), including normal-document overhead, pathological inputs, and the feature-disabled baseline comparison.
...
- **Execution**: after the existing table-length and endpoint checks, `is_in_code_range` views the range words as pairs and uses `partition_point` to find the first upper bound at or beyond the code point. The lower-bound check retains C's inclusive interval semantics. Small-table fast paths remain unchanged.
...
- **Validation and measurements**: [Unicode range lookup evaluation](/perf/unicode-range-lookup), including an exhaustive Unicode-range sweep, malformed table bounds, cache/plain/C checks, and repeated before/after workloads.
...
- **Validation and measurements**: [ASCII class run evaluation](/perf/ascii-class-runs). Tests compare raw results, captures, and deterministic limit outcomes against unbatched instructions, including malformed UTF-8 and backward or bounded searches. The opt-in cache and C differential suite also exercise the new opcode.
...
- **Validation and measurements**: [atomic class prefix evaluation](/perf/atomic-class-prefix), including 500,736 exact original-path comparisons, C/cache differential cases, repeated general workloads, and broader scanner/search/compilation checks.
