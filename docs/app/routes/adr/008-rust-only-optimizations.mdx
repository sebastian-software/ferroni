...
#### Second 2026-09 pass (`src/regset.rs`, `src/regcomp.rs`, `src/regexec.rs`, `src/regparse.rs`, `src/literal_trie.rs`)
...
- **Fast-path census**: `grammar_fast_path_census` asserts how many grammar patterns reach each of these paths, fused look-behinds included.
- **Impact**: warm TypeScript line -50%, Rust document -32%, TypeScript document -20%, CSS grammar compile -26%, TypeScript compile -17% (wall clock).
