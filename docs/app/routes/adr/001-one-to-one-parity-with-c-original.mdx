---
order: 1
title: "1:1 Parity Goal with C Original"
---

# 1:1 Parity Goal with C Original
...
6. **An idiomatic Rust API layer** wraps the C-ported internals without modifying them ([ADR-005](../005-idiomatic-rust-api-layer)). This provides `Regex::new()`, typed errors, and safe result types while keeping the internal 1:1 structure intact.
...
- New features or optimizations should first be contributed upstream to C Oniguruma, then ported — not invented in the Rust port. (Amended by [ADR-008](../008-rust-only-optimizations): additive, guarded optimizations for TextMate workloads are permitted under defined criteria.)
