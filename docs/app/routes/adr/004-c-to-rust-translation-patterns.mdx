...
- The public entry point `onig_new()` returns `Result<RegexType, RegexError>`, where `RegexError` groups ~100 error codes into semantic variants ([ADR-005](../005-idiomatic-rust-api-layer)).
